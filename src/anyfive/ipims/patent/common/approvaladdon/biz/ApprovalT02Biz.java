...
import anyfive.ipims.patent.common.approvaladdon.dao.ApprovalT02Dao;
...
public class ApprovalT02Biz extends NAbstractServletBiz implements ApprovalAbstractBiz
{
    public ApprovalT02Biz(NServiceResource nsr)
    {
        super(nsr);
    }
    ...
    {
        ApprovalT02Dao dao = new ApprovalT02Dao(this.nsr);
        ...
        NSingleData consultInfo = dao.retrieveTMarkIntConsult(apprKey);
        ...
        if (examResult.equals("1") == true) {
            ...
            if (officeCode.equals("") != true) {
                dao.updateTMarkIntConsult(apprKey);
            }
        }

        NSingleData masterInfo = dao.retrieveTMarkIntMaster(apprKey);

        // 포기인 경우
        if (examResult.equals("2")) {
            masterInfo.setString("ABD_YN", "1");
            masterInfo.setString("ABD_USER", masterInfo.getString("JOB_MAN"));
            masterInfo.setString("ABD_MEMO", "국내상표출원품의 포기");
            masterInfo.setString("ABD_DATE", NDateUtil.getSysDate());
        }

        // 국내상표출원 마스터 수정
        dao.updateTMarkIntMaster(apprKey, masterInfo);
