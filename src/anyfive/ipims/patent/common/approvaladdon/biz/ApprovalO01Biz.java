...
public class ApprovalO01Biz extends NAbstractServletBiz implements ApprovalAbstractBiz
{
    public ApprovalO01Biz(NServiceResource nsr)
    {
        super(nsr);
    }
    ...
    private void executeAgreeAll(NSingleData apprKey) throws NException
    {
        ...
        if (mainInfo.getString("EXAM_RESULT").equals("1")) {
            docBiz.setEvent("REJECT_REASON_EXAM_SEND"); // 거절이유 검토 후 의견서제출
            docBiz.setValue("REMARK", mainInfo.getString("COPE_PLAN"));
        } else {
            docBiz.setEvent("REJECT_REASON_EXAM_ABANDON"); // 거절이유 검토 후 포기
