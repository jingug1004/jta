/**
 * dsgridDnD plug-in for JQuery, allows you to drag and drop table rows
 ...
 *
 * Inside the onDrop method you can also call $.dsgridDnD.serialize() this returns a string of the form
 ...
 *                         Added dsgridDnDupate() and dsgridDnDSerialize() to be called when you are outside the table
...
jQuery.dsgridDnD = {
    ...
    build: function(options) {
        ...
        this.each(function() {
            ...
            $.dsgridDnD.makeDraggable(this);
            // Prepare hierarchy support
            this.dsgridDnDConfig.hierarchyLevel
                && $.dsgridDnD.makeIndented(this);
        });
    ...
    makeIndented: function (table) {
        var config = table.dsgridDnDConfig,
            rows = table.rows,
        ...
        config.dragHandle
            ...
            && $(config.dragHandle, table).each(function() {
                ...
                $(this).bind(startEvent, function(e) {
                    $.dsgridDnD.initialiseDrag($(this).parents('tr')[0], table, this, e, config);
                    return false;
                });
            })
            ...
            || $(table.rows).each(function() {
                ...
                if ($(this).data("tableDnD") === true) {
                    if ($(this).data("tableDnD.activated") !== true) {
                        $(this).bind(startEvent, function(e) {
                            if ($(e.target).css("cursor") == "move") {
                                $.dsgridDnD.initialiseDrag(this, table, this, e, config);
                                return false;
                            }
                        }).children('th,td').each(function() {
                            $(this).css("cursor", $(this).data("tableDnD") === true ? "move" : "");
                        }); // Store the dsgridDnD object
                        $(this).data("tableDnD.activated", true);
                    }
                } else {
    ...
    autoScroll: function (mousePos) {
      var config       = this.currentTable.dsgridDnDConfig,
    ...
    mousemove: function(e) {
        var dragObj      = $($.dsgridDnD.dragObject),
            config       = $.dsgridDnD.currentTable.dsgridDnDConfig,
            currentRow,
            mousePos,
            moving,
            x,
        ...
        mousePos = $.dsgridDnD.mouseCoords(e);
        x = mousePos.x - $.dsgridDnD.mouseOffset.x;
        y = mousePos.y - $.dsgridDnD.mouseOffset.y;

        // auto scroll the window
        $.dsgridDnD.autoScroll(mousePos);

        currentRow = $.dsgridDnD.findDropTargetRow(dragObj, y);
        moving = $.dsgridDnD.findDragDirection(x, y);

        $.dsgridDnD.moveVerticle(moving, currentRow);
        $.dsgridDnD.moveHorizontal(moving, currentRow);

        return false;
    },
    findDragDirection: function (x,y) {
        var sensitivity = this.currentTable.dsgridDnDConfig.sensitivity,
            oldX        = this.oldX,
            oldY        = this.oldY,
    ...
    findDropTargetRow: function(draggedRow, y) {
        var rowHeight = 0,
            ...
            config    = this.currentTable.dsgridDnDConfig,
            rowY      = 0,
            row       = null;
    ...
    jsonize: function(pretify) {
        ...
        if (pretify)
            return JSON.stringify(
                ...
                null,
                table.dsgridDnDConfig.jsonPretifySeparator
            );
    ...
    serializeTable: function(table) {
        var result = "";
        var paramName = table.dsgridDnDConfig.serializeParamName || table.id;
        var rows = table.rows;
        for (var i=0; i<rows.length; i++) {
            if (result.length > 0) result += "&";
            var rowId = rows[i].id;
            if (rowId && table.dsgridDnDConfig && table.dsgridDnDConfig.serializeRegexp) {
                rowId = rowId.match(table.dsgridDnDConfig.serializeRegexp)[0];
...
jQuery.fn.extend(
    {
        dsgridDnD             : $.dsgridDnD.build,
        dsgridDnDUpdate       : $.dsgridDnD.updateTables,
        dsgridDnDSerialize    : $.proxy($.dsgridDnD.serialize, $.dsgridDnD),
        dsgridDnDSerializeAll : $.dsgridDnD.serializeTables,
        dsgridDnDData         : $.proxy($.dsgridDnD.tableData, $.dsgridDnD)
    }
);
