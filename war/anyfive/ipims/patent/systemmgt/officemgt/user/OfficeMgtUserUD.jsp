...
function fnRetrieve()
{
    ...
    prx.path = top.getRoot() + "/anyfive.ipims.patent.systemmgt.officemgt.user.act.RetrieveOfficeMgtUser.do";
...
function fnSave()
{
    ...
    var prx = new any.proxy();
    prx.path = top.getRoot() + "/anyfive.ipims.patent.systemmgt.officemgt.user.act.UpdateOfficeMgtUser.do";
    prx.addParam("USER_ID", parent.USER_ID);
...
function fnDelete()
{
    ...
    var prx = new any.proxy();
    prx.path = top.getRoot() + "/anyfive.ipims.patent.systemmgt.officemgt.user.act.DeleteOfficeMgtUser.do";
    prx.addParam("USER_ID", parent.USER_ID);
...
//비밀번호 오류횟수 초기화
function fnResetPwErrCnt()
{
    if (!confirm("비밀번호 오류횟수를 0으로 초기화하시겠습니까?")) return;

    var prx = new any.proxy();
    prx.path = top.getRoot() + "/anyfive.ipims.patent.systemmgt.officemgt.user.act.UpdateUserMstPwErrCnt.do";
    prx.addParam("USER_ID", parent.USER_ID);

    prx.onSuccess = function()
    {
        alert("<%= app.message.get("msg.com.info.save").toJS() %>");
        PW_ERR_CNT.innerText = 0;
    }
...
<TABLE border="0" cellspacing="1" cellpadding="2" class="main">
    ...
    <TR>
        <TD>비밀번호 만료일자</TD>
        <TD>
            <ANY:SPAN id="PW_END_DATE" format="date" />
        </TD>
        <TD>비밀번호 오류횟수</TD>
        <TD>
            <ANY:SPAN id="PW_ERR_CNT" format="number2" />회
            <BUTTON text="오류횟수 초기화" onClick="javascript:fnResetPwErrCnt();"></BUTTON>
        </TD>
    </TR>
</TABLE>
