...
function fnSearch()
{
    var ldr = gr_countryList.loader;

    ldr.init();
    ldr.path = top.getRoot() + "/anyfive.ipims.patent.common.popup.search.act.RetrieveCountrySearchList.do";
    ldr.addParam("SEARCH_TEXT", txt_searchText.value);
    ldr.addParam("OL_GRP_ID", parent.OL_GRP_ID);
...
function fnConfirm()
{
    cfPopupSearchResult(parent, gr_countryList);
}
</SCRIPT>
</HEAD>
...
<TABLE border="0" cellspacing="0" cellpadding="0" width="100%" height="100%">
    ...
    <TR>
        <TD height="100%">
            <ANY:GRID id="gr_countryList" pagingType="0"><COMMENT>
                addColumn({ width:30 , align:"center", type:"check" , sort:false, id:"ROW_CHK"      , hide:true });
                addColumn({ width:100, align:"center", type:"string", sort:false, id:"COUNTRY_CODE" , name:"코드" });
                addColumn({ width:200, align:"left"  , type:"string", sort:false, id:"COUNTRY_NAME" , name:"국가명" });
                messageSpan = "spn_gridMessage";

                if (parent.multiCheck == true) {
                    fg.ColHidden(fg.ColIndex("ROW_CHK")) = false;
                } else {
                    addAction("COUNTRY_NAME", fnConfirm);
                }
            </COMMENT></ANY:GRID>
        </TD>
    </TR>
