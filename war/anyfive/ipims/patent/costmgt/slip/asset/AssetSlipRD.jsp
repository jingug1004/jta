...
function fnRetrieve()
{
    ...
    prx.path = top.getRoot() + "/anyfive.ipims.patent.costmgt.slip.proc.act.RetrieveSlipProcAssetNo.do";
    prx.addParam("SLIP_ID", parent.SLIP_ID);
    prx.addParam("ACCOUNT_PROC_YN", ACCOUNT_PROC_YN.value);
    prx.checkData = ds_mainInfo;

    prx.onSuccess = function()
    {
        cfShowObjects([btn_confirm,btn_line2], ds_mainInfo.value(0, "FLAG2") == "" && ds_mainInfo.value(0, "ASSET_NO") != "" && ds_mainInfo.value(0, "COST_DATE") != "" && ds_mainInfo.value(0, "INVOICE_DATE") != ""&& ds_mainInfo.value(0, "ACCOUNT_PROC_YN") != 1);

         if(ds_mainInfo.value(0, "FLAG2") == "S"){
            btn_delete.style.display = "none";
            btn_modify.style.display = "none";
        }

         if(ds_mainInfo.value(0, "FLAG2") == "I" || ds_mainInfo.value(0, "FLAG2") == "S" || ds_mainInfo.value(0, "FLAG2") == "A" || ds_mainInfo.value(0, "ACCOUNT_PROC_YN") == 1){
             btn_confirm.style.display = "none";
         }


       cfShowObjects([btn_sapCancel, btn_line3], ds_mainInfo.value(0, "FLAG2") == "I" || ds_mainInfo.value(0, "FLAG2") == "A");

        cfShowObjects([btn_line1,btn_assetNo,btn_line4], ds_mainInfo.value(0, "FLAG") == "" || ds_mainInfo.value(0, "FLAG") == "D");
    }
...
//전표 비용목록 조회
function fnRetrieveSlipCostList()
{
    var ldr = gr_slipCostList.loader;

    ldr.init();
    ldr.path = top.getRoot() + "/anyfive.ipims.patent.costmgt.slip.asset.act.RetrieveAssetSlipCostList.do";
...
function fnModify()
{
    window.location.href = "AssetSlipU.jsp";
}
...
function fnAssetNo()
{
    if (!confirm("자산번호 생성 하시겠습니까 ?")) return;

    var prx = new any.proxy();
    prx.path = top.getRoot() + "/anyfive.ipims.patent.costmgt.slip.proc.act.CreateAssetNo.do";
    prx.addParam("SLIP_ID", parent.SLIP_ID);
    prx.addParam("SLIP_KIND", "ASSET_NO");
...
<TABLE border="0" cellspacing="0" cellpadding="0" width="100%" height="100%">
    <TR>
        <TD>
            <TABLE border="0" cellspacing="1" cellpadding="2" class="main">
                ...
                <TR>
                    <TD>완료처리여부</TD>
                    <TD><ANY:SPAN id="ACCOUNT_PROC_YN" codeData="{ACCOUNT_PROC_YN}"/></TD>
                    <TD>완료처리자</TD>
                ...
                <TR style="display:none">
                    <TD>전표처리여부</TD>
                    <TD><ANY:SPAN id="ACCOUNT_PROC_YN" /></TD>
                </TR>
            </TABLE>
        </TD>
    </TR>
    <TR>
        <TD>
            <DIV class="button_area">
                ...
                <BUTTON text="자산번호생성" onClick="javascript:fnAssetNo();" id="btn_assetNo" display="none"></BUTTON>
    ...
    <TR>
        <TD height="100%">
            <ANY:GRID id="gr_slipCostList" pagingType="0" minHeight="200px"><COMMENT>
