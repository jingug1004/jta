...
function fnRetrieve()
{
    ...
    ldr.init();
    ldr.path = top.getRoot() + "/anyfive.ipims.patent.costmgt.slip.tax.act.RetrieveTaxSlipList.do";
...
function fnCreateSlip()
{
    ...
    if (!confirm("출원 수수료비용 전표를 작성하시겠습니까?")) return;

    var prx = new any.proxy();
    prx.path = top.getRoot() + "/anyfive.ipims.patent.costmgt.slip.proc.act.CreateSlipProc.do";
    prx.addParam("SLIP_SUBJECT", "<%= NDateUtil.getFormatDate("yyyy")%>년 <%= NDateUtil.getFormatDate("MM")%>월 수수료 비용");
    prx.addParam("SLIP_KIND", "T");
