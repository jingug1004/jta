...
function fnRetrieve()
{
    ...
    prx.onSuccess = function()
    {
        ...
        cfShowObjects([btn_divApply, btn_line1], ds_mainInfo.value(0, "APP_NO") != "" && ds_mainInfo.value(0, "DIVISION_LEVEL") < 2 && isPatGroupUser);
        cfShowObjects([btn_extApplyGrp], ds_mainInfo.value(0, "ABD_YN") == "0" && ds_mainInfo.value(0, "EXT_APP_STATUS") == "0" && isPatGroupUser);
        cfShowObjects([btn_extApplyAbd], ds_mainInfo.value(0, "EXT_APP_STATUS") != "1" && isPatGroupUser);
        cfShowObjects([btn_line2], btn_extApplyGrp.display != "none" || btn_extApplyAbd.display != "none");
        cfShowObjects([btn_modify, btn_line3], isPatGroupUser);
...
function fnDivApply()
{
    var path = top.getRoot() + "/anyfive/ipims/patent/applymgt/intpatent/masterdiv/IntPatentMasterDivC.jsp";
...
//해외출원품의
function fnExtApplyGrp()
{
    var path = top.getRoot() + "/anyfive/ipims/patent/applymgt/extpatent/group/ExtPatentGroupC.jsp?REF_NO=" + ds_mainInfo.value(0, "REF_NO");
...
//해외출원포기
function fnExtApplyAbd()
{
    var win = new any.window();

    if (ds_mainInfo.value(0, "EXT_APP_STATUS") == "9") {
        win.path = top.getRoot() + "/anyfive/ipims/patent/applymgt/intpatent/extabd/ExtApplyAbdR.jsp";
    } else {
        win.path = top.getRoot() + "/anyfive/ipims/patent/applymgt/intpatent/extabd/ExtApplyAbdU.jsp";
    }

    win.arg.REF_ID = parent.REF_ID;
    win.arg.REF_NO = ds_mainInfo.value(0, "REF_NO");
    win.opt.width = 700;
    win.opt.height = 350;

    if (win.show() != "OK") return;
...
function fnModify()
{
    window.location.href = top.getRoot() + "/anyfive/ipims/patent/applymgt/intpatent/master/IntPatentMasterU.jsp";
}
</SCRIPT>
</HEAD>
...
<DIV class="button_area">
    <BUTTON text="분할/우선권 출원" onClick="javascript:fnDivApply();" id="btn_divApply" ></BUTTON>
    <BUTTON auto="line" id="btn_line1" display="none"></BUTTON>
    <BUTTON text="해외출원품의" onClick="javascript:fnExtApplyGrp();" id="btn_extApplyGrp" display="none"></BUTTON>
    <BUTTON text="해외출원포기" onClick="javascript:fnExtApplyAbd();" id="btn_extApplyAbd" display="none"></BUTTON>
